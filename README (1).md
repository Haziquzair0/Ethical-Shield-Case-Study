...
| Cross-Domain Misconfiguration                     | Medium       | Medium        | `https://style.iium.edu.my/css/iium.css` | The CORS misconfiguration on the web server permits cross-domain read requests from arbitrary third party domains, using unauthenticated APIs on this domain                 | CWE-264 | WASC-4  |
| Strict-Transport-Security Header Not Set           | Low          | High       | `https://ifisonline.iium.edu.my/hr-api` | No HSTS header in response                        | CWE-319 | WASC-15 |
| Application Error Disclosure                       | Low          | Medium     | `https://ifisonline.iium.edu.my/travellingadmin/jwt` | This page contains an error/warning message that may disclose sensitive information like the location of the file that produced the unhandled exception                 | CWE-550 | WASC-13 |
| X-Content-Type-Options Header Missing              | Low          | Medium     | `https://ifisonline.iium.edu.my/travelling/callback` | Header missing in response                        | CWE-693  | WASC-15 |
| Timestamp Disclosure - Unix             | Low          | Low     | `https://ifisonline.iium.edu.my/travelling/static/js/main.64fdb97e.js` | A timestamp was disclosed by the application/web server                        | CWE-497  | WASC-13 |
| Server Leaks Version Information via "Server" HTTP Response Header Field              | Low          | High    | `https://style.iium.edu.my/vendor/bootstrap-select/dist/css/bootstrap-select.min.css` | The web/application server is leaking version information via the "Server" HTTP response header                        | CWE-497  | WASC-13 |
| Cross-Domain JavaScript Source File Inclusion              | Low          | Medium     | `http://ifisonline.iium.edu.my/travelling` | The page includes one or more script files from a third-party domain                       | CWE-829  | WASC-15 |
| Information Disclosure - Suspicious Comments       | Info         | Low        | `http://ifisonline.iium.edu.my/travelling` | Sensitive comments in JS file                     | CWE-615 | WASC-13 |
| Modern Web Application              | Info         | Medium     | `http://ifisonline.iium.edu.my/travelling`     | The application appears to be a modern web application               | CWE- | WASC- |
| Re-examine Cache-control Directives                | Info         | Low      | `https://ifisonline.iium.edu.my/travelling/manifest.json` | The cache-control header has not been set properly or is missing          | CWE-525 | WASC-13  |
| Retrieved from Cache                | Info         | Medium       | `https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css` | The content was retrieved from a shared cache         | CWE- | WASC-  |
